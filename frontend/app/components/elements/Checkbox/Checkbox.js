...
const HiddenCheckbox = ({inputRef, ...props}) => {
    return <input type="checkbox" ref={inputRef} className={styles.hidden} {...props}/>
}

const StyledCheckbox = ({children, checked}) => {
    return <div
        className={cn(styles.styled, checked ? styles.selected : styles['not-selected'])}>{checked && children}</div>
}
