...
const HiddenCheckbox = ({inputRef, ...props}) => {
    return <input type="checkbox" defaultValue="false" ref={inputRef} className={styles.hidden} {...props}/>
}

const StyledCheckbox = ({children, checked}) => {
    return <div className={cn(styles.styled,checked ? styles.selected : styles['not-selected'])}>{checked && children}</div>
}
...
const Checkbox = ({ name, label, control, defaultValue, ...props }) => (
        <Controller control={control} defaultValue={defaultValue || false} render={({field}) => (
