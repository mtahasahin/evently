...
const User = ({ user, type }) => {
  ...
  const acceptRequest = async () => {
    AxiosInstance.put(`/profile/follower-request/${user.username}`).then(
      (res) => {
  ...
  const removeRequest = async () => {
    AxiosInstance.delete(`/profile/follower-request/${user.username}`).then(
      (res) => {
        setShowButtons(false);
      }
    );
  ...
  return (
    <div className="flex flex-col items-center col-span-12 sm:col-span-6 lg:col-span-3  flex justify-center bg-white shadow rounded p-10">
      <Link href={`/@${user.username}`}>
        <a>
          <div className="flex flex-col items-center">
            ...
            <div className="text-xl hover:underline font-semibold text-center">
              {user.name}
            </div>
      ...
      {showButtons && (
        <div className="flex mt-2 gap-1">
          <Button appearance="primary" onClick={acceptRequest}>
            Accept
          </Button>
          <Button appearance="danger" onClick={removeRequest}>
            Remove
          </Button>
        </div>
      )}
...
const ShowMoreButton = ({ onClick }) => {
  return (
    <div
      onClick={onClick}
      className={`py-2 w-1/2 rounded text-gray-500 hover:bg-gray-200 text-center border border-gray-300 select-none cursor-pointer`}
    >
      Show more users
    </div>
  );
            ...
            <div className="flex flex-col gap-1 justify-center items-center bg-gray-100 my-6 p-6">
              <div className="w-full grid gap-6 grid-cols-12 my-6">
                {result.content.map((e) => (
                  <User user={e} type={type} key={e.id} />
                ))}
              </div>
