...
const CloseAccountPage = () => {
    ...
    const onSubmit = data => {
        EditApi.closeAccount({...data}).then(res => {
            logout();
        }).catch(err => {
        ...
        <div className="shadow-sm">
            <div className="bg-white rounded-t h-full w-full py-10 px-8">
                <div className="flex flex-col">
                    ...
                    Thank you for using Evently. If there is anything we can do to keep you with us, please let us know.
                </div>
            </div>
            <div className="w-full px-8 py-6 bg-gray-50 rounded-b flex flex-row justify-end">
                <Button onClick={() => setIsOpen(true)} appearance="dark" size="lg">Close Account</Button>
            </div>
            <Modal isOpen={modalIsOpen} setOpen={setIsOpen}>
                <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="p-4 border-b flex justify-between">
                        <div className="text-lg font-bold">Close Account</div>
                        <IoIosClose size="1.5rem" className="cursor-pointer" onClick={() => setIsOpen(false)}/>
                    </div>
                    <div className="p-4 border-b flex flex-col">
                        <div className="pb-5">You need to enter your password to close your account.</div>
                        <FormGroup>
                            <div className="w-full flex flex-col gap-y-2">
                                <TextInput type="password" register={register} name="password"
                    ...
                    <div className="p-4  flex flex-row justify-end gap-x-2">
                        <Button type="button" onClick={() => setIsOpen(false)} size="lg"
                                appearance="white">Cancel</Button>
                        <Button size="lg" appearance="danger">Close Account</Button>
                    </div>
                </form>
            </Modal>
        </div>
