        ...
        <FormGroup>
          <div className="flex flex-col space-y-2">
            ...
            <div className="text-xs">
              Attendees will be on the{' '}
              <span className="font-semibold">waitlist</span> and should be
              approved by organizers before the event.
            </div>
          </div>
        </FormGroup>
        <FormGroup>
...
const CreateEditEventPage = () => {
  ...
  }, [event, edit, isError, isLoading, reset]);

  const isOnlineEvent = useWatch({
    control: control,
    name: 'eventLocationType',
  });
  ...
  const onSubmit = (data) => {
    setSubmitting(true);
    if (edit) {
      EventApi.editEvent({ slug, ...data })
        .then((res) => {
          toast('Event updated.', {
            type: 'success',
          });
          reload(res.data, false).then(() => {
