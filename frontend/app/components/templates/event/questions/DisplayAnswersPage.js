        ...
        <div className="pt-3">
            {type === "SHORT_TEXT" &&
                <div className="w-full">
                    <input type="text" className={textInputStyles.base} readOnly defaultValue={answer || ""}/>
                </div>}

            {type === "LONG_TEXT" &&
                <div className="w-full">
                    <textarea className={textAreaStyles.base} readOnly defaultValue={answer || ""}/>
                </div>}
        </div>
    </div>
}
...
const DisplayAnswersPage = () => {
    ...
    const setApproved = () => {
        setEventApplication(application => ({...application, has_approved: true}));
    }


    useEffect(() => {
        if (slug && applicationId) {
            EventApi.getAnswer({slug, applicationId})
                .then(res => {
                    const application = res.data.data;
                    application.answers.forEach(answer => {
                        application.questions.find(question => question.id === answer.questionId).answer = answer.answer;
                    })
    ...
    return (
        !eventApplication ? <LoadingIndicator isLoading={true}/> :
            <div className="flex flex-col space-y-5">
                <EventApplication application={eventApplication} slug={slug} showAnswersPage={false}
                ...
                <div className="flex flex-col ml-8 space-y-5">
                    {eventApplication.questions.map(question => (
                        <Question key={question.id} title={question.title} description={question.description}
                                  isRequired={question.required} type={question.type} answer={question.answer}/>
                    ))}
                </div>
            </div>
    );
