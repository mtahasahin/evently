...
const Alert = () => {
  ...
  return (
    show && (
      <div className="w-full relative flex flex-row p-5 rounded bg-yellow-100 text-yellow-900 border border-yellow-200">
        <div>
        ...
        <div className="w-full mx-3 opacity-80">
          ...
          <div className="text-sm mb-1">
            Please answer the required questions below to attend this event. We
            will get you notified when organizer approve your attendance.
          </div>
        </div>
        <div>
          <button
            ...
            className="absolute top-0 right-0 p-3 hover:text-black"
          >
            <MdClose />
          </button>
        </div>
      ...
      .catch((res) => {
        ...
        res.response.data.errors?.forEach((error) => {
          setQuestions((questions) => {
            const question = questions.indexOf(
              questions.find((q) => q.id == error.field)
            );
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="flex flex-col space-y-5">
        ...
        {!!questions.length ? (
          ...
          <span className="text-gray-500 text-center text-sm p-5">
            No questions
          </span>
        )}
      </div>
