...
const CalendarMenu = ({ date }) => {
  return (
    <div className="relative h-[72px] w-[64px] min-w-[64px] bg-white rounded border select-none cursor-pointer">
...
const EventTitle = ({ title, slug }) => {
  return (
    <Link href={`/event/${slug}`}>
      <a className="text-3xl font-semibold hover:underline">{title}</a>
    </Link>
  );
};
