...
const SearchBox = () => {
  ...
  const { data, error } = useSWR(
    () => (debouncedQuery && menuOpened ? ['/search', debouncedQuery] : null),
    (url, query) =>
    ...
    <>
      <div className="relative" ref={wrapperRef}>
        <div className="h-full navbar-search-wrapper relative ml-4">
          <label className="absolute left-3 top-2 bottom-2">
            <AiOutlineSearch
        ...
        <div
          className={`absolute ${
            menuOpened && debouncedQuery.length >= 2
              ? 'flex flex-col'
              : 'hidden'
          } ml-[-100px] justify-between bg-white shadow mt-2 w-[600px] left-auto rounded z-10`}
        >
          {isLoading && (
          ...
          {!isLoading && (
            <>
              ...
              <div className="bg-gray-50 border-t p-3 text-gray-700 text-sm">
                <Link href={`/search/${debouncedQuery}`}>
                  <a onClick={closeMenu}>
                    Show more results for "{debouncedQuery}"
                  </a>
                </Link>
              </div>
            </>
